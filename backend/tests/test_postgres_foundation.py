...
def test_migration_files_are_contiguous() -> None:
    assert [path.name for path in migration_files()] == [
        ...
        "0041_modular_rag_runtime.sql",
        "0042_bad_case_regression_evidence.sql",
    ]
...
def test_required_schema_version_matches_the_migration_files() -> None:
    ...
    assert Settings(_env_file=None).required_database_schema_version == len(migration_files())
...
def test_schema_two_with_existing_data_upgrades_to_schema_three(tmp_path: Path) -> None:
    ...
    assert apply_migrations(database_url) == 42
    check_schema_version(database_url, 42)
    with psycopg.connect(database_url) as connection:
        ...
        source_path, version_count, job_count = connection.execute(
            """SELECT dv.source_path,
                      ...
                      (SELECT count(*) FROM index_jobs j
                         JOIN document_versions all_dv
                           ON all_dv.document_version_id=j.document_version_id
                        WHERE all_dv.document_id=d.document_id)
