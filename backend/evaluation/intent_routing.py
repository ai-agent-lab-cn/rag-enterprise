...
def evaluate_intent_router(
    ...
    for sample in samples:
        ...
        if actual != expected:
            failures.append(
                {"sample_id": str(sample["sample_id"]), "expected": expected, "actual": actual}
            )
    per_intent = {label: _f1(label, truth, predicted) for label in labels}
    macro_f1 = sum(per_intent.values()) / len(per_intent)
    control_precision = control_correct / control_predicted if control_predicted else (1.0 if not control_true else 0.0)
