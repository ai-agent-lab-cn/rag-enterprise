...
export function ParsingPanel({ knowledgeBaseId, versions, canManage, onRefresh }: { knowledgeBaseId: string; versions: DocumentVersion[]; canManage: boolean; onRefresh: () => Promise<void> }) {
  ...
  return <section className="parsing-panel">
    ...
    {preview && canManage ? <div className="chunking-policy"><strong>切片策略</strong><label>目标长度<Input size="sm" type="number" className="w-24" min={100} max={4000} value={chunkSize} onChange={(event) => setChunkSize(Number(event.target.value))}/></label><label>重叠长度<Input size="sm" type="number" className="w-24" min={0} max={1000} value={chunkOverlap} onChange={(event) => setChunkOverlap(Number(event.target.value))}/></label><Button size="sm" loading={reprocessing} blockedReason={chunkOverlap >= chunkSize ? "重叠必须小于切片大小" : undefined} onClick={() => void reprocess()}>{reprocessing ? "已排队…" : "重新处理"}</Button><small>修改参数后需重新处理，成功前仍使用当前可用索引。</small></div> : null}
