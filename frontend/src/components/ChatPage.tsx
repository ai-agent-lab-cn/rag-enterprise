...
function EvidencePanel({ sources, documents, loading, activeRecord, conversation, result, onUpload, onDeleteDocument }: EvidencePanelProps) {
  const [tab, setTab] = useState<"sources" | "documents">("sources");
  const model = result?.model ?? activeRecord?.models.generation ?? "尚未生成回答";
  return (
    <aside className="evidence-panel" aria-label="引用来源与知识库资料">
      <div className="evidence-tabs" role="tablist">
        <button className={tab === "sources" ? "is-active" : ""} onClick={() => setTab("sources")} role="tab">引用来源 <span>{sources.length}</span></button>
        <button className={tab === "documents" ? "is-active" : ""} onClick={() => setTab("documents")} role="tab">资料库 <span>{documents.length}</span></button>
      </div>
      {tab === "sources" ? <>
        <div className="evidence-heading"><h2>引用来源</h2><small>按相关度展示</small></div>
        <div className="evidence-list">{sources.length ? sources.map((source, index) => <SourceCard source={source} index={index} defaultOpen key={source.chunk_id}/>) : <div className="evidence-empty"><FileText size={28}/><strong>回答后查看证据</strong><p>答案引用的文件、位置与原文会显示在这里。</p></div>}</div>
      ...
      </> : <DocumentPanel documents={documents} loading={loading} onUpload={onUpload} onDelete={onDeleteDocument}/>}
    </aside>
  );
}
...
export function ChatPage({ conversationId, onOpen }: { conversationId?: string; onOpen: (path: string) => void }) {
  ...
  const upload = async (file: File) => { setBusy(true); setError(""); try { await api.uploadKnowledgeBaseDocument(baseId, file); await loadBase(baseId); } catch (reason) { setError(reason instanceof Error ? reason.message : "上传失败。"); } finally { setBusy(false); } };
  ...
  return <div className="chat-layout">
    <aside className="history-panel">
      ...
      <label className="base-picker"><span>当前知识库</span><select aria-label="当前知识库" value={baseId} onChange={(event) => selectBase(event.target.value)}>{bases.map((item) => <option key={item.knowledge_base_id} value={item.knowledge_base_id}>{item.name}</option>)}</select></label>
    ...
    <div className="chat-stage">
      <section className="conversation">
        <header className="conversation-header"><div><h1>{history?.title ?? "新对话"}</h1><p>{bases.find((item) => item.knowledge_base_id === baseId)?.name ?? "当前知识库"}</p></div><div className="conversation-actions"><button disabled={!history} onClick={exportConversation}><Download size={16}/> 导出</button><button disabled={!conversationId} onClick={() => setConfirmDelete(true)}><Trash2 size={16}/> 清空</button></div></header>
        ...
        <div className="composer-wrap"><div className="examples">{EXAMPLES.map((item) => <button key={item} onClick={() => setQuestion(item)}>{item}</button>)}</div><form className="question-box" onSubmit={ask}><label className="sr-only" htmlFor="question">向知识库提问</label><textarea id="question" value={question} onChange={(event) => setQuestion(event.target.value)} placeholder="输入你的问题…" rows={2} maxLength={2000}/><div className="question-footer"><span><Paperclip size={16}/>{documents.length ? `已连接 ${documents.length} 份资料` : "请先在右侧资料库上传文档"}</span><button aria-label="提问并发送" disabled={busy || !question.trim()}>{busy ? <span className="button-spinner"/> : <Send size={17}/>}</button></div></form><small className="ai-notice">内容由人工智能生成，请注意甄别信息准确性</small></div>
      </section>
      <EvidencePanel sources={sources} documents={documents} loading={busy} activeRecord={activeRecord} conversation={history} result={result} onUpload={upload} onDeleteDocument={remove}/>
    </div>
