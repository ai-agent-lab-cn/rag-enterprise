  ...
  return <div className="chat-layout">
    ...
    <div className="chat-stage">
      <section className="conversation">
        <header className="conversation-header"><h1>{history?.title ?? "新对话"}</h1><div className="conversation-actions"><Button variant="outline" size="sm" reasonHidden blockedReason={history ? undefined : "当前对话还没有内容"} onClick={exportConversation}><Download size={16}/> 导出对话</Button><Button variant="outline" size="sm" reasonHidden className="text-danger-text hover:bg-danger-subtle" blockedReason={conversationId ? undefined : "当前是新对话，没有内容可清空"} onClick={() => setConfirmDelete(true)}><Trash2 size={16}/> 清空对话</Button></div></header>
        <div className="conversation-scroll">
          {/* 弹层开着时错误只在弹层里显示：这条横幅在 Radix 的 aria-hidden 背景里。 */}
          {error && !confirmDelete ? <div className="error-banner" role="alert">{error}</div> : null}
        ...
        <div className="composer-wrap"><div className="examples">{EXAMPLES.map((item) => <Button variant="outline" size="sm" key={item} onClick={() => setQuestion(item)}>{item}</Button>)}</div><form className="question-box" onSubmit={ask}>{/* label 文字要 shrink-0：它和 w-full 的输入框同处一个 flex，不锁住就会被挤成两行。 */}<div className="query-filters" aria-label="检索过滤条件"><label><span className="shrink-0">分类</span><Select size="sm" aria-label="过滤分类" value={categoryFilter} onChange={(event) => setCategoryFilter(event.target.value)}><option value="">全部分类</option>{categories.map((item) => <option key={item.category_id} value={item.category_id}>{item.name}{item.active ? "" : "（已停用）"}</option>)}</Select></label><label><span className="shrink-0">标签</span><Input size="sm" aria-label="过滤标签" value={tagFilter} onChange={(event) => setTagFilter(event.target.value)} placeholder="逗号分隔"/></label><label><span className="shrink-0">来源</span><Select size="sm" aria-label="过滤来源类型" value={sourceTypeFilter} onChange={(event) => setSourceTypeFilter(event.target.value)}><option value="">全部</option><option value="file">文件</option><option value="object_storage">对象存储</option><option value="web">网页</option><option value="connector">连接器</option></Select></label></div><label className="sr-only" htmlFor="question">向知识库提问</label><textarea id="question" value={question} onChange={(event) => setQuestion(event.target.value)} placeholder="输入你的问题…" rows={2} maxLength={2000}/><div className="question-footer"><div className="composer-context"><label className="base-picker"><span>知识库：</span><Select size="sm" aria-label="当前知识库" value={baseId} onChange={(event) => selectBase(event.target.value)}>{bases.map((item) => <option key={item.knowledge_base_id} value={item.knowledge_base_id}>{item.name}</option>)}</Select></label><span className="connected-documents"><Paperclip size={15}/>{documents.length ? `已连接 ${documents.length} 份资料` : "暂无资料"}</span></div>{/* reasonHidden：这是个只有图标的按钮，「还没输入问题」旁边就是那个空的输入框。 */}<Button type="submit" size="icon" aria-label="提问并发送" loading={busy} reasonHidden blockedReason={question.trim() ? undefined : "请先输入问题"}>{busy ? <span className="button-spinner"/> : <Send size={17}/>}</Button></div></form><small className="ai-notice">内容由人工智能生成，请注意甄别信息准确性</small></div>
      </section>
      <EvidencePanel sources={sources} documents={documents} activeRecord={activeRecord} conversation={history} result={result}/>
    </div>
    {confirmDelete ? <Dialog open title="清空当前会话" description="删除后无法恢复，知识库资料不会受到影响。" onClose={() => { if (!busy) setConfirmDelete(false); }}>{error ? <div className="error-banner" role="alert">{error}</div> : null}<DialogActions><Button variant="secondary" loading={busy} onClick={() => setConfirmDelete(false)}>取消</Button><Button variant="destructive" autoFocus loading={busy} onClick={() => void removeConversation()}>确认删除</Button></DialogActions></Dialog> : null}
  </div>;
}
