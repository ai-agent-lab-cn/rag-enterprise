...
const METRIC_ROWS: Array<{ key: "recall_at_5" | "vector_mrr" | "rerank_mrr"; label: string }> = [
...
function MetricCard({ label, metric }: { label: string; metric: EvaluationMetric }) {
  ...
  return (
    <article className="quality-card">
      <div className="quality-heading"><span>{label}</span><strong>{metric.value.toFixed(4)}</strong></div>
  ...
  return (
    <section className="evaluation-page" aria-labelledby="evaluation-title">
      <header className="page-heading">
        <div><span className="eyebrow">只读质量证据</span><h1 id="evaluation-title">检索评测</h1></div>
      ...
      {reports?.length === 0 ? <div className="evaluation-state"><h2>还没有正式评测报告</h2><p>报告需要由离线真实模型评测生成，页面不会启动重量评测任务。</p></div> : null}

      {reports && reports.length > 0 ? (
        <>
          <label className="report-picker">评测运行
            <select value={selectedId} onChange={(event) => {
              setReport(null);
              setError("");
              setSelectedId(event.target.value);
            }}>
              {reports.map((item) => <option key={item.report_id} value={item.report_id}>{new Date(item.run_at).toLocaleString("zh-CN")} · {item.dataset_version}</option>)}
            </select>
          </label>
      ...
      {report ? (
        <div className="evaluation-content">
          <div className="report-context">
            <div><span>数据版本</span><strong>{report.dataset_version}</strong></div>
            <div><span>标注问题</span><strong>{report.query_count} 条</strong></div>
            <div><span>运行时间</span><strong>{new Date(report.run_at).toLocaleString("zh-CN")}</strong></div>
            <div><span>代码提交</span><a href={`https://github.com/ai-agent-lab-cn/rag-enterprise/commit/${report.commit}`} target="_blank" rel="noreferrer">{report.commit.slice(0, 8)}</a></div>
          </div>
          <div className="quality-grid">
            {METRIC_ROWS.map(({ key, label }) => <MetricCard key={key} label={label} metric={report[key]} />)}
          </div>
          <div className="report-details">
            <section><span className="section-kicker">模型标识</span>{Object.entries(report.models).map(([key, value]) => <p key={key}><b>{key}</b><code title={value}>{value}</code></p>)}</section>
            <section><span className="section-kicker">运行参数</span><dl>{Object.entries(report.parameters).map(([key, value]) => <div key={key}><dt>{key}</dt><dd>{String(value)}</dd></div>)}</dl></section>
          </div>
