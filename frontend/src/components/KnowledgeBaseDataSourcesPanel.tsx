...
export function KnowledgeBaseDataSourcesPanel({ knowledgeBaseId, items, categories, onRefresh }: Props) {
  ...
  return <section className="source-governance">
    <div className="source-toolbar"><button className="primary-action" type="button" onClick={openCreate}>新建外部数据源</button></div>
      ...
      <tbody>{items.map((item) => {
        ...
        return <tr key={item.data_source_id}>
          ...
          <td><div className="table-actions">
            {item.source_type === "object_storage" && item.allowed_actions.includes("edit") ? <button disabled={!!busyId || syncing} onClick={() => openEdit(item)}>编辑</button> : null}
            {item.allowed_actions.includes("test") ? <button disabled={!!busyId} onClick={() => void act(item.data_source_id, () => api.testDataSource(item.data_source_id), "连接测试通过。")}>测试连接</button> : null}
            {item.allowed_actions.includes("sync") ? <button disabled={!!busyId || syncing} onClick={() => void act(item.data_source_id, () => api.syncDataSource(item.data_source_id), "同步任务已进入队列。")}>同步</button> : null}
            <button onClick={() => void openHistory(item)}>记录</button>
            {item.sync_status === "failed" || item.sync_status === "aborted" ? <button disabled={!!busyId} onClick={() => void act(item.data_source_id, () => api.retryDataSource(item.data_source_id), "失败同步已重新入队。")}>重试</button> : null}
            {item.allowed_actions.includes(item.enabled ? "disable" : "enable") ? <button disabled={!!busyId || syncing} onClick={() => void act(item.data_source_id, () => api.setDataSourceEnabled(item.data_source_id, !item.enabled), item.enabled ? "数据源已停用。" : "数据源已启用。")}>{item.enabled ? "停用" : "启用"}</button> : null}
            {item.allowed_actions.includes("delete") ? <button className="danger-link" disabled={!!busyId} onClick={() => { if (window.confirm(`确认删除数据源“${item.name}”？`)) void act(item.data_source_id, () => api.deleteDataSource(item.data_source_id), "数据源已删除。"); }}>删除</button> : null}
          </div></td>
        </tr>;
      })}</tbody>
    </table></div> : <p className="empty-copy">当前知识库没有数据源。</p>}
    {formOpen ? <Modal title={editing ? "编辑 S3 兼容数据源" : "新建 S3 兼容数据源"} description="凭据只读取运行环境变量，不保存到数据库。" onClose={() => { if (!busyId) { setFormOpen(false); setEditing(null); } }}>
      <form className="modal-form" onSubmit={(event) => { event.preventDefault(); void save(); }}>
        ...
        <label>默认分类<select value={draft.categoryId} onChange={(event) => setDraft({ ...draft, categoryId: event.target.value })}><option value="">未分类</option>{categories.filter((item) => item.active).map((item) => <option key={item.category_id} value={item.category_id}>{item.name}</option>)}</select></label>
        ...
        <footer className="modal-actions"><button className="secondary-action" type="button" disabled={!!busyId} onClick={() => setFormOpen(false)}>取消</button><button className="primary-action" type="submit" disabled={!!busyId}>{busyId ? "保存中…" : "保存"}</button></footer>
      </form>
    </Modal> : null}
    {historyFor ? <Modal title="同步记录" description={historyFor.name} onClose={() => setHistoryFor(null)}><div className="sync-run-list">{runs.length ? runs.map((run) => <div key={run.sync_run_id}><strong>{run.status} · {run.stage}</strong><span>新增 {run.added_count} / 更新 {run.updated_count} / 删除 {run.deleted_count} / 跳过 {run.skipped_count} / 失败 {run.failed_count}</span><small>{new Date(run.created_at).toLocaleString("zh-CN")}{run.failure_reason ? ` · ${run.error_code || "ERROR"}: ${run.failure_reason}` : ""}</small></div>) : <p className="empty-copy">暂无同步记录。</p>}</div></Modal> : null}
  </section>;
}
